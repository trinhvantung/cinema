...
public class VnPayServiceImpl implements VnPayService {
    ...
    @Override
    public String getPaymentUrl(String ipAddress, BigDecimal amount, Integer bookingId, Integer userId) {
        ...
        params.put("vnp_Amount", amount.multiply(BigDecimal.valueOf(100)).stripTrailingZeros().toPlainString());
        ...
        for (Map.Entry<String, String> entry : params.entrySet()) {
            try {
                paramList.add(entry.getKey() + "=" + URLEncoder.encode(entry.getValue(), StandardCharsets.US_ASCII.toString()));
    ...
    public Map<String, String> refund(Integer bookingId) {
        ...
        Payment payment = paymentRepository.findByBookingIdAndPaymentStatus(bookingId, PaymentStatus.SUCCESS).orElseThrow();
        ...
        params.put("vnp_Amount", payment.getAmount().multiply(BigDecimal.valueOf(100)).stripTrailingZeros().toPlainString());
        ...
        VnPayRefundResponseDTO response = restTemplate.postForObject(vnPayConfig.getRefundUrl(), params, VnPayRefundResponseDTO.class);
    ...
    public Map<String, String> refundForCancellingBooking(Integer bookingId) {
        ...
        if (response.getResponseCode().equals("00")) {
            payment.setPaymentStatus(PaymentStatus.REFUNDED);
            kafkaTemplate.send("refund-for-cancelling-result", new RefundResultForCancellingEvent(bookingId, true));
        } else {
            payment.setPaymentStatus(PaymentStatus.REFUND_FAILED);
            kafkaTemplate.send("refund-for-cancelling-result", new RefundResultForCancellingEvent(bookingId, false));
        }
