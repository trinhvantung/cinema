...
public class MovieServiceImpl implements MovieService {
    ...
    public void create(MultipartFile thumbnailFile, MovieRequestDTO request) {
        ...
        if (thumbnailFile == null || thumbnailFile.getOriginalFilename().isBlank()) {
            throw new ApplicationException("thumnail_empty", "Thumnail empty");
        }
        ...
        kafkaTemplate.send("create-movie", new MovieCreatedEvent(movieMapper.movieToMovieResponseDTO(savedMovie)));
    }
    ...
    public void update(Integer movieId, MultipartFile thumbnailFile, MovieRequestDTO request) {
        ...
        request.getCategories().forEach(categoryId -> movie.getCategories().add(Category.builder().id(categoryId).build()));

        Movie savedMovie = movieRepository.save(movie);


        kafkaTemplate.send("update-movie", new MovieCreatedEvent(movieMapper.movieToMovieResponseDTO(savedMovie)));
    }
    ...
    public List<MovieResponseDTO> getAll() {
        return movieRepository.findAll().stream().map(movieMapper::movieToMovieResponseDTO).collect(Collectors.toList());
    }
