...
import { Button, Descriptions, Modal, Popconfirm, Table, Tag, Typography, notification } from 'antd';
...
const OrderItem = ({ item }) => {
    ...
    const handleCancelOrder = order => {
        console.log(order)
        bookingApi.cancelling(order.id).then(res => {
    ...
    return (
        <div className='order_item'>
            <div className="image">
                <div style={{ backgroundImage: `url(${item.show.movie.thumbnailUrl})` }}></div>
            </div>
            <div className="info">
                <div>
                    ...
                    <div className="hall">
                        <p>Giờ chiếu:</p>
                        <p>{formatDate(item.show.start)} - {formatDate(item.show.end)} <span style={{
                            fontWeight: 600,
                            color: '#000'
                        }}>({gettDate(item.show.start)})</span></p>
                    </div>

                    <div className="status">
                        <p>Trạng thái: </p>
                        <p><Tag color={item.status == 'COMPLETED' ? 'green' : 'red'}>{getOrderStatus(item.status)}</Tag></p>
                    </div>
                </div>
                <div>
