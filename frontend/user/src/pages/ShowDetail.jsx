...
const getTime = (dateString) => {
    ...
    const formattedDate = `${hours}:${minutes} - ${isToday ? "Hôm nay," : ''} ${day}/${month}`
        ...
        const detail = seatClicked.reduce((result, value) => {
            ...
            if (!temp) {
                ...
                result.push({
                    ...
                    seats: [value.name],
                    totalPrice: showPrice.find(sp => sp.seatTypeId === value.seatType.id).price
                })
            } else {
                result = result.map(i => {
                    if (i.seatType.id === value.seatType.id) {
                        const totalPrice = i.totalPrice + showPrice.find(sp => sp.seatTypeId === value.seatType.id).price
                        return {
                            ...i,
        ...
        if (show) {
            ...
            for (let i = 1; i <= maxRowIndex; i++) {
                ...
                for (let j = 1; j <= maxColumnIndex; j++) {
                    temp.push(show.seats.find(s => s.columnIndex === j && s.rowIndex == i))
                }
                dataTemp.push(temp)
            }
                        ...
                        <Col span={16}>
                            <div className={`${step === 2 ? 'd-none' : ''}`}>
                                <div style={{ overflow: 'hidden' }} ref={refDetail}>
                                    <div style={{ transform: `scale(${scale}) translate(${pos.x}px, ${pos.y}px)` }}
                                        onWheel={handleWheel}
                                            ...
                                            <div className='data'>
                                                {
                                                    data.map((dataItem, idx) => (
                                                        <div className="seats" key={idx}>
                                                            {
                                                                dataItem.map((item, index) => <div key={index} className='seat-row'>
                                                                    {
                                                                        item.map((s, id) => s ? <Seat
                                                                            booked={bookedSeat.find(bs => bs.seat.id === s.id)}
                                                                            key={id} onClick={() => handleClickSeat(s)} seat={s} /> :
                                    ...
                                    <Row gutter={24} justify={'center'}>
                                        <Col>
                                            <div className="seat_type_item">
                                                <div className="color" style={{ backgroundColor: '#e5e5e5' }}></div>
                                                <div className="name">Đã chọn</div>
                                            </div>
                                        </Col>
                                        <Col>
                                            <div className="seat_type_item">
                                                <div className="color" style={{ backgroundColor: '#555555', color: 'white' }}></div>
                                                <div className="name">Đã bán</div>
                                            </div>
                                        </Col>
                                        {
                                            seatType.map(st => <Col key={st.id}>
                                                <div className="seat_type_item">
                                                    <div className="color" style={{ backgroundColor: `${st.color}` }}></div>
                            ...
                            <Space direction="vertical" size={16} style={{ width: '100%' }} className={`${step === 1 ? 'd-none' : ''}`}>
                                <Card
                        ...
                        <Col span={8}>
                            <div className="show_info">
                                <div className="show_info_item">
                                    <div>Phim:</div><div>{show ? show.movie.name : ''}</div>
                                </div>
                                <div className="show_info_item">
                                    <div>Phòng chiếu:</div><div>{show ? show.hall.name : ''}</div></div>
                                <div className="show_info_item">
                                    <div>Giờ chiếu:</div><div>{show ? getTime(show.start) : ''}</div>
                                </div>
                                <div className="show_info_item">
                                    <div>Rạp:</div><div>{show ? show.hall.cinema.name : ''}</div>
                                </div>
                                <div className="show_info_item">
                                    <div>Ghế:</div><div>{seatClicked.length > 0 ? seatClicked.map(s => s.name).join(", ") : "Chưa chọn"}</div>
                                </div>
                            </div>
                            ...
                            {
                                step === 1 && <Button type='primary' size='large' style={{ width: '100%' }}
                                    onClick={() => {
                                    ...
                                    disabled={!totalPrice}>Tiếp tục</Button>
                            }
                            {
                                step === 2 && (
                                    <>
                                        <Button onClick={handleBooking} type='primary' size='large' style={{ width: '100%' }}>Thanh toán</Button>
                                        <Button type='default' size='large' style={{ width: '100%', marginTop: 12 }}
