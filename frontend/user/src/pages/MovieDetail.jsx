...
const MovieDetail = () => {
    ...
    const handleReview = data => {
        data.star = star
        reviewApi.create(id, data).then(res => {
            console.log(res)
    ...
    return (
        <Container>
            <div className="movie_detail">
                <div className="image"><div style={{ backgroundImage: `url(${movie.thumbnailUrl})` }}></div></div>
                <div className="movie_info">
                    ...
                    <div className="release_date">
                        <div>Ngày phát hành: </div> {getReleaseDate(movie.releaseDate || "")}
                    </div>
                    <div className="categories">
                        <div>Danh mục: </div>{movie.categories && movie.categories.map(c => c.name).join(", ")}
                    </div>
                    <p>Nội dung:</p>
            ...
            <div className="show">
                ...
                <div className="show_body">
                    ...
                    <div className="right" style={{ borderRight: '1px solid rgba(20, 20, 20, .07)' }}>
                    ...
                    <div className="left">
                        <div className="cinema_search_location">
                            <p style={{ fontSize: 18, fontWeight: 'bold' }}>Đánh giá ({reviewDetail ? reviewDetail.starResult : 5} sao)</p>

                        </div>

                        {
                            localStorage.getItem("accessToken") ? (
                                <div className="cinema_search" style={{ paddingBottom: 0, border: 'none' }}>
                                    <div>
                                        <Form
                                            form={reviewForm}
                                            onFinish={handleReview}
                                        >
                                            <Form.Item style={{ marginBottom: 6 }} name={'content'} rules={[
                                                {
                                                    required: true,
                                                ...
                                                <TextArea value={contentValue}
                                                    ...
                                                    placeholder='...' rows={4} style={{ resize: 'none' }} disabled={review} />
                                            </Form.Item>
                                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', top: 0 }}>
                                                <Rate value={review ? review.star : star} style={{ marginBottom: 12 }}
                                                    onChange={(v) => setStar(v)} disabled={review} />
                                                {
                                                    !review && (
                                                        <Form.Item>
                                                            <Button htmlType="submit" loading={null} onClick={null} type="primary" style={{ marginLeft: 'auto' }}>
                                                                Đánh giá
                                                            </Button>
                                                        </Form.Item>
                                                    )
                                ...
                                <div style={{
                                    ...
                                    padding: 24,
                                    textAlign: 'center'
                                }}>Đăng nhập để đánh giá</div>
                            )
                        }
                        <div style={{ display: 'flex', justifyContent: 'center', fontSize: 8, marginTop: 8 }}>
                            <div value={'1'} onChange={null} style={{ padding: '0 12px 12px 12px', display: 'flex', justifyContent: 'center', rowGap: 6, columnGap: 12, flexWrap: 'wrap', fontSize: 8 }}>
                                <Button
                                    ghost={currentStar === 0}
                                    type={currentStar === 0 ? 'primary' : 'default'}
                                    value="1" onClick={() => handleGetReviewByStar(0)}>
                                    Tất cả ({reviewDetail ? reviewDetail.totalReview : 0})</Button>
                                <Button
                                    ghost={currentStar === 5}
                                    type={currentStar === 5 ? 'primary' : 'default'}
                                    value="2" onClick={() => handleGetReviewByStar(5)}>
                                    5 sao ({reviewDetail ? reviewDetail.reviewStarCounts[0].quantity : 0})</Button>
                                <Button
                                    ghost={currentStar === 4}
                                    type={currentStar === 4 ? 'primary' : 'default'}
                                    value="3" onClick={() => handleGetReviewByStar(4)}>
                                    4 sao ({reviewDetail ? reviewDetail.reviewStarCounts[1].quantity : 0})</Button>
                                <Button
                                    ghost={currentStar === 3}
                                    type={currentStar === 3 ? 'primary' : 'default'}
                                    value="4" onClick={() => handleGetReviewByStar(3)}>
                                    3 sao ({reviewDetail ? reviewDetail.reviewStarCounts[2].quantity : 0})</Button>
                                <Button
                                    ghost={currentStar === 2}
                                    type={currentStar === 2 ? 'primary' : 'default'}
                                    value="5" onClick={() => handleGetReviewByStar(2)}>
                                    2 sao ({reviewDetail ? reviewDetail.reviewStarCounts[3].quantity : 0})</Button>
                                <Button
                                    ghost={currentStar === 1}
                                    type={currentStar === 1 ? 'primary' : 'default'}
                                    value="6" onClick={() => handleGetReviewByStar(1)}>
                                    1 sao ({reviewDetail ? reviewDetail.reviewStarCounts[4].quantity : 0})</Button>
                            </div>
                        </div>
                                ...
                                <div className="comment_list">
                                    {
                                        reviews.map(r => (
                                            <Comment
                                                key={r.id}
                                                style={{ padding: 12 }}
                                                actions={[<Rate disabled defaultValue={r.star} style={{ fontSize: 16 }} />]}
                                                author={<a>{r.user.fullName}</a>}
                                                avatar={<Avatar
                                                    style={{
                                                        backgroundColor: getColorAndName(r.user.fullName).color,
                                                        verticalAlign: 'middle',
                                                    }}
                                                    size="large"
                                                >
                                                    {getColorAndName(r.user.fullName).name}
                                                </Avatar>}
                                                content={
                                                    <p>{r.content}</p>
                                                }
                                                datetime={
                                                    <Tooltip title={r.createdDate}>
